        ...
        for _loc in range(REPEAT_DIFFERENT_LOC):
            ...
            for _i in range(REPEAT_STATS):
                ...
                for policy in POLICIES:
                    ...
                    print(
                        "Map:{map} policy:{policy} loc:{loc_repeat} @{start},{goal} for {repeating}".format(
                            map=test_map,
                            policy=policyname,
                            loc_repeat=_loc,
                            start=start,
                    ...
                    args = [
                        "python",
                        os.path.join(CUR_PATH, "benchmark_rrt_wrapper.py"),
                        policy,
                        test_map,
                        "start",
                        str(start[0]),
...
def get_random_free_space(image):
    ...
    while True:
        new_p = (
            int(random.random() * image.get_width()),
            int(random.random() * image.get_height()),
        )
