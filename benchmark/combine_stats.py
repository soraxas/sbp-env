...
def combine_result(folder):
    ...
    def create_main_stats():
        ...
        for i in range(start, end):
            ...
            for j, policy in enumerate(POLICIES):
                ...
                for local_col_idx, col in enumerate(
                    range(INFO_MAX_COL + 2, INFO_MAX_COL + INFO_EXTRA_COL * 2 + 2)
                ):
                    # Set titles
            ...
            chart = build_scatter_with_mean_stdev(
                xvalues_refs=xvalues,
    ...
    POLICIES = set()
    POLICIES.update(
        re.search("policy=(.*)_timestamp", fn).group(1)
    ...
    for idx, policy in enumerate(POLICIES):
        sheets, num_rows = save_csv_as_sheet(
            ...
            get_sheetname_func=lambda fn, policy=policy: "{}_{}".format(
                policy,
                re.search(".*timestamp=[0-9]+-([0-9]{4})[0-9]{2}.csv", fn).group(1),
            ),
            row_filter=lambda row_idx, row, z=DATA_BEGIN_ROW: row
            if row_idx <= z
...
def create_rescaled_sampled_pt(wb, sheets, num_rows, stat_ws):
    ...
    for s in sheets:
        valid_sampled_pts = [
            c[0].value for c in wb[s]["A5:A" + str(num_rows)] if c[0].value is not None
        ]
        invalid_pts_con = [
            c[0].value for c in wb[s]["D5:D" + str(num_rows)] if c[0].value is not None
        ]
        invalid_pts_obs = [
            c[0].value for c in wb[s]["E5:E" + str(num_rows)] if c[0].value is not None
        ]
        # print(s)
    ...
    for sampled_pts, _, _ in stats:
        max_sampled_size = (
            sampled_pts[-1] if sampled_pts[-1] > max_sampled_size else max_sampled_size
        )
    ...
    stats_connection = (
        np.mean(normalized_stats_con, axis=0),
        np.std(normalized_stats_con, axis=0),
    )
    stats_obstacle = (
        np.mean(normalized_stats_obs, axis=0),
        np.std(normalized_stats_obs, axis=0),
    )
...
def create_stats_time_taken(wb, raw_sheets, stat_ws):
    ...
    def find_time_taken(ws):
        ...
        for r in range(DATA_BEGIN_ROW + 1, ws.max_row + 1):
            if (
                solution_found is None
                and ws.cell(row=r, column=DATA_COST_COL).value != "inf"
            ):
                # first time the policy found a solution
                # Index 1 as node count, index 2 as node cost
                solution_found = (
                    ws.cell(row=r, column=DATA_NUMNODE_COL).value,
    ...
    for nodesnum, cost in ((0, 1), (2, 3)):
        main_stats.append(
            statistics.mean(stats[nodesnum]) if len(stats[nodesnum]) > 0 else -1
        )
        main_stats.append(
            statistics.stdev(stats[nodesnum]) if len(stats[nodesnum]) > 1 else -1
        )
...
def create_stats_view(main_ws, sheets, num_rows):
    ...
    for column in range(1, INFO_MAX_COL + 1):
        for row in range(1, INFO_END_ROW + 1):
            val = RefFormula(worksheet=sheets[0], min_col=column, min_row=row)
    ...
    val = RefFormula(worksheet=sheets[0], min_col=1, min_row=row)
    ...
    duplicate_col(
        ...
        ws=main_ws,
        row=row,
        name_generator=lambda sh=sheets[0], data=(
            "(mean)",
            "(stdev)",
    ...
    for row in range(DATA_BEGIN_ROW + 1, num_rows + 1):
        val = "='{sheet}'!{c}{r}".format(sheet=sheets[0], c="A", r=row)
        main_ws.cell(row=row, column=1, value=val)
        ...
        duplicate_col(
            ...
            ws=main_ws,
            row=row,
            name_generator=lambda sheet_begin=sheets[0], sheet_end=sheets[-1], data=(
                "AVERAGE",
                "STDEV",
